...
export const COMMAND_GROUP_META: CommandGroupMeta[] = [
  ...
  },
  {
    ...
    noteEn:
      'Requires admin. Regular players get a failure — the same code as a typo, indistinguishable.',
    collapsed: true
  }
]
...
export { LAYOUT_OPTIONS, LAYOUT_VALUES } from '@/lib/layouts'
...
const C = (
  c: Omit<CatalogCommand, 'aliases' | 'admin'> & Partial<Pick<CatalogCommand, 'aliases' | 'admin'>>
): CatalogCommand => ({
  aliases: [],
  admin: false,
  ...c
...
export const COMMAND_CATALOG: CatalogCommand[] = [
  /* ── 闪退自救 ── */
  C({
    cmd: '/log attack clear',
    syntax: '/log attack clear',
    group: 'rescue',
    desc: '进攻日志闪退时清空进攻日志'
  }),
  C({
    cmd: '/log activity clear',
    syntax: '/log activity clear',
    group: 'rescue',
    desc: '活动日志闪退时清空活动日志'
  }),
  C({
    cmd: '/map deepsea reset',
    syntax: '/map deepsea reset',
    group: 'rescue',
    desc: '点潜水艇闪退时重置潜水艇'
  }),
  C({
    cmd: '/cleararts',
    syntax: '/cleararts <player>',
    group: 'rescue',
    admin: true,
  ...
  C({ cmd: '/olp', syntax: '/olp', group: 'basic', queryOnly: true, desc: '全服在线玩家列表' }),
  C({
    cmd: '/say',
    syntax: '/say <文本>',
    aliases: ['/broadcast'],
  ...
  C({
    cmd: '/bebean',
    syntax: '/bebean',
    aliases: ['/Bebean', '/bot', '/robot'],
    group: 'basic',
    desc: '介绍本服机器人 Bebean'
  }),
  C({ cmd: '/utc', syntax: '/utc', aliases: ['/UTC'], group: 'basic', desc: '介绍 UTC 时间' }),
  C({
    cmd: '/log activity',
    syntax: '/log activity <summary|invade|deepsea|event|enemy>',
    group: 'basic',
    desc: '按类型查看活动日志'
  }),
  C({
    cmd: '/log notice',
    syntax: '/log notice <clear|custom|gem|chest>',
    group: 'basic',
    desc: '通知日志：清空或插入一条测试通知'
  }),
  C({
    cmd: '/log notice resource',
    syntax: '/log notice <resource> <数量>',
    group: 'basic',
    desc: '插入一条资源变动通知'
  }),
  C({
    cmd: '/log warship clear',
    syntax: '/log warship clear',
    group: 'basic',
    desc: '清空战斗母舰日志'
  }),
  ...
  C({
    cmd: '/attackplayer',
    syntax: '/attackplayer [<player>]',
    aliases: ['/attackp', '/atkp'],
    group: 'battle',
    desc: '进攻某玩家的岛屿，留空则打自己'
  }),
  C({
    cmd: '/visitplayer',
    syntax: '/visitplayer [<player>]',
    aliases: ['/visitp', '/scoutplayer', '/scoutp'],
    group: 'battle',
    desc: '侦察某玩家的岛屿，留空则看自己'
  }),
  C({
    cmd: '/attackwarship',
    syntax: '/attackwarship [<player>]',
    aliases: ['/attackw', '/atkw'],
    group: 'battle',
    desc: '进攻战斗母舰基地'
  }),
  C({
    cmd: '/visitwarship',
    syntax: '/visitwarship [<player>]',
    aliases: ['/visitw', '/scoutwarship', '/scoutw'],
    group: 'battle',
    desc: '侦察战斗母舰基地'
  }),
  C({ cmd: '/atkpr', syntax: '/atkpr', group: 'battle', desc: '随机挑一个已缓存的玩家来打' }),
  C({
    cmd: '/replay',
    syntax: '/replay [<回放ID>]',
    group: 'battle',
  ...
  C({
    cmd: '/homege',
    syntax: '/homege <layout> <难度>',
    group: 'battle',
    desc: '用主基地生成器随机生成一个基地给你打'
  }),
  C({
    cmd: '/homege2',
    syntax: '/homege2 <layout> <难度>',
    group: 'battle',
    desc: '同 /homege，但改为侦察'
  }),
  C({
    cmd: '/attackbuilding',
    syntax: '/attackbuilding <building>',
    aliases: ['/atkbd'],
    group: 'battle',
    desc: '生成一个铺满同种建筑的基地给你打'
  }),
  C({
    cmd: '/basebuilder',
    syntax: '/basebuilder <名字>',
    aliases: ['/bb'],
    group: 'battle',
    desc: '进攻基地编辑器做的基地'
  }),
  ...
  C({ cmd: '/newdef', syntax: '/newdef <难度>', group: 'battle', desc: '新版防御哈莫曼' }),
  C({
    cmd: '/sandbox',
    syntax: '/sandbox',
    group: 'battle',
  ...
  C({
    cmd: '/resource fill',
    syntax: '/resource fill',
    group: 'resource',
    desc: '给予大部分常用资源'
  }),
  C({ cmd: '/resource proto', syntax: '/resource proto', group: 'resource', desc: '给予原型模块' }),
  C({
    cmd: '/resource artifact',
    syntax: '/resource artifact',
    group: 'resource',
    desc: '给予神庙物品'
  }),
  C({ cmd: '/resource magic', syntax: '/resource magic', group: 'resource', desc: '给予魔法物品' }),
  C({
    cmd: '/resource clear',
    syntax: '/resource clear',
    group: 'resource',
    danger: 'destructive',
    desc: '清空全部资源'
  }),
  ...
  C({
    cmd: '/setboat',
    syntax: '/setboat <艇号> <character> <数量>',
    group: 'home',
    desc: '设置登陆艇兵种，艇号 0-7，例如 /setboat 0 [Rifleman] 50'
  }),
  C({
    cmd: '/setboat fix_pos',
    syntax: '/setboat fix_pos',
    group: 'home',
    needsHome: true,
    desc: '修正部队存储位置异常'
  }),
  ...
  C({
    cmd: '/bunker set',
    syntax: '/bunker <序号> set <character> <数量>',
    group: 'home',
    desc: '设置地堡部队'
  }),
  C({ cmd: '/bunker clear', syntax: '/bunker <序号> clear', group: 'home', desc: '清空指定地堡' }),
  C({
    cmd: '/layout',
  ...
  C({
    cmd: '/blockingmask',
    syntax: '/blockingmask <island|warship> <掩码>',
    group: 'home',
    danger: 'warn',
  ...
  C({ cmd: '/gbe', syntax: '/gbe', group: 'home', desc: '快速拿到最强战舰能量雕像' }),
  C({
    cmd: '/mutate',
    syntax: '/mutate <变异JSON>',
    group: 'home',
    danger: 'warn',
  ...
  C({
    cmd: '/prototroop fill',
    syntax: '/prototroop fill',
    group: 'home',
    desc: '给予全部原型部队'
  }),
  C({
    cmd: '/prototroop clear',
    syntax: '/prototroop clear',
    group: 'home',
    danger: 'warn',
    desc: '清空原型部队'
  }),
  ...
  C({
    cmd: '/material add',
    syntax: '/material add <matData> <statData>',
    group: 'home',
    desc: '添加指定建筑装置'
  }),
  C({
    cmd: '/material clear',
    syntax: '/material clear',
    group: 'home',
    danger: 'warn',
    desc: '清空建筑装置'
  }),
  C({
    cmd: '/engraving level',
    syntax: '/engraving level <雕刻> <值>',
    group: 'home',
    desc: '设置雕刻等级'
  }),
  C({
    cmd: '/engraving quality',
    syntax: '/engraving quality <雕刻> <值>',
    group: 'home',
    desc: '设置雕刻品质'
  }),
  C({
    cmd: '/engraving select',
    syntax: '/engraving select <雕刻>',
    group: 'home',
    desc: '选中指定雕刻'
  }),
  C({
    cmd: '/engraving fill',
    syntax: '/engraving fill',
    group: 'home',
    desc: '全部雕刻升至满级满品质'
  }),
  C({
    cmd: '/engraving clear',
    syntax: '/engraving clear',
    group: 'home',
    danger: 'destructive',
    desc: '降级全部雕刻并清空品质'
  }),
  ...
  C({
    cmd: '/coop bb',
    syntax: '/coop bb <序号> <名字>',
    group: 'map',
    desc: '把指定任务改为基地编辑器里的地图'
  }),
  C({
    cmd: '/coop rename',
    syntax: '/coop rename <序号> <名字>',
    group: 'map',
    desc: '重命名任务基地'
  }),
  ...
  C({
    cmd: '/coop mutate',
    syntax: '/coop mutate <序号> <变异>',
    group: 'map',
    desc: '变异任务基地'
  }),
  C({
    cmd: '/warship item fill',
    syntax: '/warship item fill',
    group: 'map',
    desc: '补满战斗母舰物品'
  }),
  C({
    cmd: '/warship item clear',
    syntax: '/warship item clear',
    group: 'map',
    desc: '清空战斗母舰物品'
  }),
  C({
    cmd: '/warship tech clear',
    syntax: '/warship tech clear',
    group: 'map',
    danger: 'destructive',
    desc: '重置战斗母舰科技树'
  }),
  C({
    cmd: '/warship max_score',
    syntax: '/warship max_score',
    group: 'map',
    queryOnly: true,
    desc: '查询战斗母舰最高分'
  }),
  ...
  C({
    cmd: '/sector reset_supplies',
    syntax: '/sector reset_supplies',
    group: 'map',
    desc: '清空原始水晶兑换记录'
  }),
  C({
  ...
  C({
    cmd: '/rule spell',
    syntax: '/rule spell [<spell>]',
    group: 'map',
    desc: '打 NPC 时附加额外战舰能力。只能生效一个，填多个取最后一个'
  }),
  C({
    cmd: '/rule bonus',
    syntax: '/rule bonus <加成> <千分比> [<过滤器>]',
    group: 'map',
  ...
  C({ cmd: '/supplychest reset', syntax: '/supplychest reset', group: 'map', desc: '重置补给箱' }),
  C({
    cmd: '/trader reset',
    syntax: '/trader reset',
    group: 'map',
  ...
  C({
    cmd: '/badword',
    syntax: '/badword <get|search|searchall|test> <参数>',
    group: 'admin',
    admin: true,
    queryOnly: true,
    desc: '敏感词表查询与测试'
  ...
  C({
    cmd: '/vp',
    syntax: '/vp <island|warship> <数值>',
    aliases: ['/trophy'],
    group: 'admin',
    admin: true,
    desc: '设置奖杯数。写在家园代码里，但需要管理员权限'
  }),
  C({
    cmd: '/crab',
    syntax: '/crab <阶段>',
